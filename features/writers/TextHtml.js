...
const formTableFeatureCollection=(responsePayload,serverConfig, reqparams)=> {
  ...
  if (features!==undefined && Array.isArray(features) && features.length>0){
    ...
    if (properties!==undefined && Object.entries(properties).length>0){
      ...
      for (let i=0;i<features.length;i++){
        retstr+=formTableRows4Feature(keys,features[i],serverConfig, reqparams);
      }
      retstr+='</table>'
    }
  }
...
const formTableRows4Feature=(keys,afeature,serverConfig, reqparams)=> {
  let fid=afeature.id;
...
const formFeaturePropertiesList=(responsePayload,serverConfig, reqparams)=> {
  let retstr='';
...
* */
const sendResponse=(response, payload, serverConfig, reqparams)=> {
    /**
...
//      +'  style: styleFunction,'
      ...
      if (mtype!==undefined && mtype==="FeatureCollection"){
        responseHtml+=formTableFeatureCollection(responsePayload,serverConfig, reqparams);
      }else if (mtype!==undefined && mtype==="Feature"){
        responseHtml+=formFeaturePropertiesList(responsePayload,serverConfig, reqparams);
      }
      responseHtml+='    <script>';
