...
const getFeature = async ({ providers, collectionId, featureId }) => {
  ...
  //In case, there is no ? in the string, it append it to the end.
  if (connectionString.lastIndexOf("?")<0) connectionString+='?';

  //"&STOREDQUERY_ID=http%3A%2F%2Fwww.opengis.net%2Fdef%2Fquery%2FOGC-WFS%2F0%2FGetFeatureById"
  let urlstr=connectionString+"&SERVICE=WFS&version=2.0.0&REQUEST=GetFeature&OUTPUTFORMAT=application%2Fgml%2Bxml%3B%20version%3D3.2"
...
const getFeatures = async ({ providers, collectionId, limit, bbox, datetime, startindex, resulttype,properties,sortby,select_properties,skip_geometry,q }) => {
  console.log("In getFeatures of WFS200....");
  let retdata={
    'payload':{
        'features': [],
        'links': []
