...
const GetServer = require('../models/GetServer');
...
class Controller {
  ...
  */
  ...
  static async handleRequest(request, response, serviceOperation) {
    try {
      const reqParams = this.collectRequestParams(request);
      //      const serviceResponse = await serviceOperation(this.collectRequestParams(request));
      const serviceResponse = await serviceOperation(request, reqParams);

      const responsePayload = serviceResponse.payload !== undefined
        ? serviceResponse.payload : serviceResponse;
      const mtype = responsePayload.type;

      if (mtype === undefined || mtype === null) {
        Controller.sendResponse(response, serviceResponse);
      } else {
        const serverConfig = await GetServer.getServer();
        const writer = new Writer();
        writer.sendResponse(request, response, serviceResponse,
          serverConfig.payload.server, reqParams);
      }
    } catch (error) {
